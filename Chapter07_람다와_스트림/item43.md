...
## 핵심 정리 
 * 메서드 참조는 람다의 간단명료한 대안이 될 수 있습니다. **메서드 참조 쪽이 짧고 명확하다면 메서드 참조를 쓰고, 그렇지 않을 때만 람다를 사용합시다.**
 * 다만 자바는 메서드 참조로 사용할 수 있는 메소드들이 많이 제공됩니다. **반면에 스위프트는 메서드를 직접 만들어야 합니다.** 따라서 해당 기능이 자주 쓰이는 경우라면 클로저 대신 메서드를 따로 만들어서 이용하면 될 것 같습니다.
