...
Array나 Set과 같은 컬렉션을 순회 할 때, 전통적인 for-loop을 사용할지, for-each문을 사용할지 고민됩니다. 두 방법 모두 순차적으로 element에 접근이 가능합니다. 메커니즘이 매우 유사해 선호도나 스타일에 따라 달리 사용하기도 하지만 몇가지 뚜렷한 면에서 차이가 있습니다. 이번 아이템에선 그 차이점들에 대해 알아봅니다.



### for-loop

for-loop문의 반복은 코드 제어 흐름 내에서 직접 수행되어 이러한 반복을 훨씬 더 정확하게 제어 할 수 있다는 것입니다. 예를 들어 continue 키워드를 이용해서 다음 요소로 넘어갈지 혹은 break 키워드를 이용해서 반복을 멈출지 결정할 수 있습니다.
...
for-loop문에서는 where절을 적용할 수 있어 위와 같은 코드는 아래처럼 간결하게 사용할 수도 있습니다.
```swift
...
```
이러한 흐름 제어 관련 기능(`continue`, `break`, `return`, `where`)은 for-loop문을 선택하는 데 좋은 이유가 될 수 있습니다. 하지만 이러한 수준의 제어가 필요하지 않은 경우라면 forEach를 통해 조금 더 간단한 코드를 만들 수 있습니다.
